...
CUDA buffers and streams are owned by PyTorch so the detector and MCITrack SOT
share one CUDA context. Cannot be exercised on a Windows dev box (no TRT) — it is imported
...
class TensorRTBackend(DetectorBackend):
    ...
    def _setup_io(self):
        ...
        if self._trt10:
            ...
            for n in names:
                ...
                np_dtype = np.dtype(trt.nptype(self.engine.get_tensor_dtype(n)))
                torch_dtype = torch.from_numpy(np.empty((), dtype=np_dtype)).dtype
                self._dev[n] = torch.empty(shape, dtype=torch_dtype, device="cuda")
                ctx.set_tensor_address(n, self._dev[n].data_ptr())
            ...
            for i in range(self.engine.num_bindings):
                ...
                np_dtype = np.dtype(trt.nptype(self.engine.get_binding_dtype(i)))
                torch_dtype = torch.from_numpy(np.empty((), dtype=np_dtype)).dtype
                self._dev[name] = torch.empty(
                    shape, dtype=torch_dtype, device="cuda")
                self.bindings.append(self._dev[name].data_ptr())
    ...
    def _infer(self, model_input):
        ...
        host_input = torch.from_numpy(np.ascontiguousarray(model_input))
        with torch.cuda.stream(self.stream):
            self._dev[in_name].copy_(host_input, non_blocking=True)
            stream_handle = self.stream.cuda_stream
            if self._trt10:
                if not ctx.execute_async_v3(stream_handle):
                    raise RuntimeError("TensorRT execute_async_v3 failed")
            elif not ctx.execute_async_v2(self.bindings, stream_handle):
                raise RuntimeError("TensorRT execute_async_v2 failed")
        self.stream.synchronize()
        return self._dev[self._output_name][:n].cpu().numpy()
